...
def parse_recommendation(text: str) -> tuple[str, str]:
    """Parse LLM pairwise choice output into (choice_title, explanation)."""
    choice = ""
    explanation = ""
...
def parse_user_update(text: str) -> str:
    """Parse user agent update output into the new self-description."""
    match = re.search(
...
def parse_item_update(text: str) -> tuple[str, str]:
    """Parse item agent update output into (neg_item_description, pos_item_description).

    The original prompt asks for 'first CD' (neg) and 'second CD' (pos) descriptions.
    Returns (neg_description, pos_description) to match the original ordering.
    """
    first_match = re.search(
        r"(?:The )?updated description of the first CD\s*(?:is)?:\s*(.+?)(?=(?:The )?updated description of the second|$)",
        text,
        re.IGNORECASE | re.DOTALL,
    )
    second_match = re.search(
        r"(?:The )?updated description of the second CD\s*(?:is)?:\s*(.+?)$",
        text,
        re.IGNORECASE | re.DOTALL,
    )
...
def parse_evaluation_ranking(text: str) -> list[str]:
    """Parse evaluation ranking output into ordered list of item titles."""
...
def fuzzy_match_title(query: str, candidates: list[str]) -> tuple[str, int]:
    ...
    Returns (matched_title, index_in_candidates).
    Uses longest common subsequence ratio for matching.
    """
    if not candidates:
        return "", -1
...
def _lcs_ratio(a: str, b: str) -> float:
    """Compute LCS-based similarity ratio between two strings."""
    if not a or not b:
        return 0.0
    m, n = len(a), len(b)
    if m > 500 or n > 500:
