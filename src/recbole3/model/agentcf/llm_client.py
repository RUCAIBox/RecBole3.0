...
class LLMClient:
    ...
    def _request_sync(self, url: str, payload: dict[str, Any]) -> dict[str, Any]:
        """Make a single synchronous HTTP request with retry."""
        ...
        for attempt in range(self._request_retries):
            try:
                ...
                print(f"[llm_client] request failed (attempt {attempt + 1}): {e}. Retrying in {wait:.0f}s...")
                time.sleep(wait)
        raise RuntimeError("Unreachable")

    async def _request_async(self, url: str, payload: dict[str, Any], semaphore: asyncio.Semaphore) -> dict[str, Any]:
        """Make a single async HTTP request (runs sync request in thread pool)."""
    ...
    def chat_completion(self, messages: list[dict[str, str]], *, temperature: float | None = None) -> str:
        """Single chat completion request. Returns the assistant message content."""
    ...
    def chat_completion_batch(
        ...
        *,
        ...
        """Batch chat completion requests with async concurrency."""
        ...
        async def _run() -> list[str]:
            ...
            for resp in responses:
                if isinstance(resp, Exception):
                    print(f"[llm_client] batch request failed: {resp}")
                    results.append("")
                else:
    ...
    def embedding_batch(self, texts: list[str]) -> list[list[float]]:
        ...
        async def _run() -> list[list[float]]:
            ...
            for resp in responses:
                if isinstance(resp, Exception):
                    print(f"[llm_client] embedding request failed: {resp}")
                    results.append([])
                else:
