...
class RPGSemanticTokenizer:
    ...
    def _load_or_generate_semantic_ids(self, prepared_data: BaseTaskDataset) -> dict[int, tuple[int, ...]]:
        ...
        if self.config.semantic_id_file:
            raise FileNotFoundError(f"RPG semantic_id_file does not exist: {semantic_id_path}")

        semantic_id_path.parent.mkdir(parents=True, exist_ok=True)
        if self._distributed_rank() != 0:
            return self._wait_for_semantic_id_file(semantic_id_path)

        generated = self._generate_semantic_ids(prepared_data)
        tmp_path = semantic_id_path.with_name(f"{semantic_id_path.name}.{os.getpid()}.tmp")
        with tmp_path.open("w", encoding="utf-8") as f:
            json.dump({str(k): list(v) for k, v in generated.items()}, f)
        os.replace(tmp_path, semantic_id_path)
        return generated

    def _wait_for_semantic_id_file(self, path: Path) -> dict[int, tuple[int, ...]]:
        timeout_seconds = int(self.config.semantic_id_wait_timeout_seconds)
        deadline = time.monotonic() + timeout_seconds
        while time.monotonic() < deadline:
            if path.exists():
                return self._load_semantic_ids(path)
            time.sleep(1.0)
        raise TimeoutError(
            f"Timed out waiting {timeout_seconds}s for RPG semantic id file generated by rank 0: {path}"
        )
    ...
    def _load_semantic_ids(self, path: Path) -> dict[int, tuple[int, ...]]:
        ...
        missing = [item_id for item_id in range(self.num_items) if item_id not in values]
        extra = [item_id for item_id in values if item_id < 0 or item_id >= self.num_items]
        if missing or extra:
            raise ValueError(
                "RPG semantic id file must contain exactly one entry for each prepared item id "
                f"in [0, {self.num_items - 1}], got missing={missing[:5]} extra={extra[:5]}."
            )

        item2semantic_ids: dict[int, tuple[int, ...]] = {}
        for item_id in range(self.num_items):
            raw_codes = values[item_id]
            if not isinstance(raw_codes, (list, tuple)) or len(raw_codes) != self.n_digit:
                raise ValueError(
                    f"RPG semantic ids for item {item_id} must have length {self.n_digit}, got {raw_codes!r}."
                )
            codes = tuple(int(value) for value in raw_codes)
            invalid_codes = [value for value in codes if value < 0 or value >= self.codebook_size]
            if invalid_codes:
                raise ValueError(
                    f"RPG semantic ids for item {item_id} must be in [0, {self.codebook_size - 1}], "
                    f"got invalid values {invalid_codes[:5]}."
                )
            item2semantic_ids[item_id] = codes
    ...
    def _item_metadata_texts(self, item_table: pd.DataFrame) -> list[str]:
        ...
        for row in item_table.to_dict("records"):
            ...
            if text:
                texts[item_id] = text
        missing = [item_id for item_id, text in enumerate(texts) if not text]
        if missing:
            raise ValueError(
                "RPG requires item metadata text for semantic ID generation. "
                f"Missing text for prepared item ids {missing[:5]}."
            )
        return texts

    def _metadata_text(self, row: Mapping[str, Any]) -> str:
        configured_text = self._clean_metadata_value(row.get(self.config.metadata_text_field))
        if configured_text:
            return configured_text

        parts = [
            self._clean_metadata_value(row.get(self.config.item_title_field)),
            self._clean_metadata_value(row.get(self.config.item_description_field)),
        ]
    ...
    def _get_codebook_bits(codebook_size: int) -> int:
        ...
        if codebook_size <= 0:
            raise ValueError("RPG codebook_size must be a positive power of two.")
