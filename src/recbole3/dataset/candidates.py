...
from recbole3.dataset.utils import CANDIDATE_ITEM_IDS, ITEM_ID, SEEN_ITEM_IDS, USER_ID
...
class CandidateGenerationConfig(Protocol):
    """Structural configuration contract shared by candidate-generation consumers."""
...
class BaseCandidateGenerator(ABC):
    ...
    def build_split_frame(self, split: str) -> pd.DataFrame:
        ...
        if bool(self.model_config.use_candidate_file) and not self.model_config.refresh_candidate_cache and external_path.exists():
            print(f"[candidates] split={split} source={self.source_name} file=hit path={external_path}")
        ...
        if not self.model_config.refresh_candidate_cache and self.cache.exists(cache_path):
            print(f"[candidates] split={split} source={self.source_name} cache=hit path={self.cache.path(cache_path)}")
        ...
        print(
            f"[candidates] split={split} source={self.source_name} cache=miss rows={len(source_frame)} "
        ...
        print(
            f"[candidates] split={split} source={self.source_name} cache=written rows={len(candidate_frame)} "
    ...
    def _cache_signature(self) -> str:
        payload = {
            # Preserve existing cache keys after moving candidate generation out of LLMRank.
    ...
    def _eval_frame(self, split: str) -> pd.DataFrame:
        ...
        if not isinstance(dataset, FrameDataset):
            raise TypeError(f"Candidate generation requires FrameDataset, got {type(dataset).__name__}.")
    ...
    def _align_prepared_eval_frame(self, prepared_frame: pd.DataFrame, *, split: str) -> pd.DataFrame:
        ...
        if len(filtered_frame) != len(expected_frame):
            raise ValueError(
                ...
                f"but the candidate-generation eval subset expects {len(expected_frame)} rows."
            )
...
class RandomCandidateGenerator(BaseCandidateGenerator):
    ...
    def _generate_candidates(self, eval_frame: pd.DataFrame, *, split: str) -> list[tuple[int, ...]]:
        ...
        print(f"[candidates] generating random candidates for split={split} rows={len(eval_frame)}")
...
class BM25CandidateGenerator(BaseCandidateGenerator):
    ...
    def __init__(
        ...
        *,
        ...
        print(f"[candidates] building bm25 token corpus for {len(self._item_text_lookup)} items")
        stage_start = time.perf_counter()
        self._encoded_item_text = self._load_segment_text(self._item_text_lookup)
        print(f"[candidates] tokenized bm25 corpus in {time.perf_counter() - stage_start:.2f}s")
        stage_start = time.perf_counter()
        self._bm25_model = BM25Model(self._encoded_item_text)
        print(f"[candidates] built bm25 index in {time.perf_counter() - stage_start:.2f}s")
        self._all_item_ids = np.arange(int(self.task_data.get_num_items()), dtype=np.int64)

    def _generate_candidates(self, eval_frame: pd.DataFrame, *, split: str) -> list[tuple[int, ...]]:
        candidate_rows: list[tuple[int, ...]] = []
        print(f"[candidates] generating bm25 candidates rows={len(eval_frame)}")
...
class ModelBackboneCandidateGenerator(BaseCandidateGenerator):
    ...
    def __init__(
        ...
        *,
        ...
        if self._model_spec.model_data_cls is None:
            raise TypeError(
                f"Candidate source '{self.backbone_name}' does not provide one model-side dataset adapter "
                "and cannot be used as a candidate-generation backbone."
            )
    ...
    def _generate_candidates(self, eval_frame: pd.DataFrame, *, split: str) -> list[tuple[int, ...]]:
        ...
        print(
            f"[candidates] generating {self.backbone_name} candidates for split={split} rows={len(eval_frame)} "
            f"batch_size={int(self._backbone_trainer_config.batch_size)}"
        )
    ...
    def _load_trained_backbone_model(self) -> BaseRetrievalModel:
        ...
        if checkpoint_path is None:
            print(f"[candidates] no {self.backbone_name} checkpoint provided; auto-training backbone")
        ...
        print(f"[candidates] loading {self.backbone_name} checkpoint from {checkpoint_path}")
